            ...
            <div className="dark:bg-ink-900 min-h-screen flex flex-col">
                ...
                <div className="flex-1 flex items-center justify-center px-4 py-20">
                    <div className="text-center">
                        <div className="w-16 h-16 mx-auto rounded-2xl bg-brand-50 dark:bg-brand-900 flex items-center justify-center mb-5">
                            <ShoppingBag className="text-brand-400 dark:text-brand-400" size={28} />
                        </div>
                        <h2 className="text-lg font-bold text-slate-800 dark:text-brand-50">Your cart is empty</h2>
                        <p className="text-sm text-slate-400 dark:text-brand-200/50 mt-1.5">
                            Browse listings from students on your campus and add something you like.
                        </p>
                        <Link
                            to="/browse"
                            className="inline-flex items-center gap-1.5 mt-6 px-5 py-2.5 rounded-xl bg-brand-600 dark:bg-brand-500 hover:bg-brand-700 dark:hover:bg-brand-400 text-white dark:text-ink-900 font-semibold text-sm transition shadow-sm"
                        >
                            Browse listings →
                        </Link>
                    </div>
                ...
                <div className="max-w-5xl mx-auto px-3 sm:px-6 py-4 sm:py-8 grid lg:grid-cols-3 gap-6 pb-24 lg:pb-8">
                    ...
                    <div className="lg:col-span-2 space-y-3 overflow-x-hidden">
                        {items.map((item) => (
                            <div
                                key={item.product_id}
                                className="flex items-center gap-2 sm:gap-4 bg-white dark:bg-ink-800 border border-slate-200 dark:border-ink-600 rounded-2xl p-2 sm:p-3 shadow-sm hover:shadow-md dark:hover:shadow-brand-900/20 transition w-full"
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="font-semibold text-slate-800 dark:text-brand-50 text-xs sm:text-sm truncate">{item.title}</p>
                                    <p className="text-brand-700 dark:text-brand-400 font-bold text-xs sm:text-sm mt-0.5">GHS {parseFloat(item.price).toFixed(2)}</p>
                                </div>
                                <div className="flex items-center gap-1 sm:gap-3 bg-slate-50 dark:bg-ink-700 rounded-full px-1.5 sm:px-3 py-1 sm:py-1.5 shrink-0">
                                    <button
                                        ...
                                        className="w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center rounded-full hover:bg-white dark:hover:bg-ink-600 text-slate-600 dark:text-brand-200 transition"
                                    >
                                        <Minus size={12} className="sm:size-[13px]" />
                                    </button>
                                    <span className="text-xs sm:text-sm font-semibold w-4 text-center text-slate-800 dark:text-brand-50">{item.quantity}</span>
                                    <button
                                        onClick={() => updateQuantity(item.product_id, item.quantity + 1)}
                                        className="w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center rounded-full hover:bg-white dark:hover:bg-ink-600 text-slate-600 dark:text-brand-200 transition"
                                    >
                                ...
                                <button
                                    ...
                                    className="text-slate-300 dark:text-brand-200/40 hover:text-red-500 transition p-1 shrink-0"
                                    title="Remove"
                                >
                        ...
                        {deliveryMethod === 'pickup' && (
                            <div className="bg-white dark:bg-ink-800 border border-slate-200 dark:border-ink-600 rounded-2xl p-4 mt-2">
                                <p className="text-xs font-semibold text-slate-400 dark:text-brand-200/50 uppercase tracking-wide mb-3">
                                    Chat with sellers
                                </p>
                                        ...
                                        return (
                                            <div key={group.sellerName + (group.whatsapp || '')} className="flex gap-2">
                                                <button
                                                    ...
                                                    className="flex-1 flex items-center justify-between gap-2 px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl border border-brand-200 dark:border-brand-800 bg-brand-50 dark:bg-brand-900/30 hover:bg-brand-100 dark:hover:bg-brand-900/50 text-brand-800 dark:text-brand-300 text-xs sm:text-sm font-semibold transition"
                                                >
                                                ...
                                                >
                                                    <WhatsAppIcon className={`w-4 h-4 sm:w-5 sm:h-5 ${href ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-300 dark:text-brand-200/30'}`} />
                                                </a>
                                            </div>
                                        );
                                    })}
                    ...
                    <div className="lg:col-span-1">
                        <div className="lg:sticky lg:top-24 bg-white dark:bg-ink-800 border border-slate-200 dark:border-ink-600 rounded-2xl p-4 sm:p-6 shadow-sm">
                            <h3 className="font-bold text-slate-900 dark:text-brand-50 mb-4">Order summary</h3>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <button
                                        ...
                                        className={`flex flex-col items-center gap-2 py-4 rounded-xl border text-sm font-semibold transition ${
                                            deliveryMethod === 'pickup'
                                                ? 'border-brand-500 dark:border-brand-500 bg-brand-50 dark:bg-brand-900/40 text-brand-700 dark:text-brand-300'
                                                : 'border-slate-200 dark:border-ink-600 text-slate-500 dark:text-brand-200/50 hover:border-slate-300 dark:hover:border-ink-500'
                                        }`}
                                    >
                                        <MapPin size={18} />
                                        Meet on campus
                                    ...
                                    <div className="px-3 py-2.5 rounded-xl border border-slate-200 dark:border-ink-600 bg-slate-50 dark:bg-ink-700">
                                        <label className="flex items-center gap-2.5 cursor-pointer">
                                            <input
                                                ...
                                                className="w-4 h-4 rounded accent-brand-600 dark:accent-brand-500"
                                            />
                                            <span className="text-sm font-medium text-slate-700 dark:text-brand-100">
                                                Are you on campus?
                                            </span>
                                        </label>

                                        {verifyingCampus && (
                                            <p className="text-xs text-slate-400 dark:text-brand-200/50 mt-2 flex items-center gap-1.5">
                            ...
                            {deliveryMethod === 'delivery' && confirmedOnCampus !== true && (
                                <p className="text-xs text-slate-400 dark:text-brand-200/50 mt-3 mb-1">Delivered within 1–3 working days.</p>
                            )}

                            <div className="border-t border-slate-100 dark:border-ink-600 pt-4 space-y-2.5">
                                <div className="flex items-center justify-between text-sm text-slate-500 dark:text-brand-200/60">
                            ...
                            <div className="border-t border-slate-100 dark:border-ink-600 mt-4 pt-4 flex items-center justify-between mb-6">
                                <span className="font-semibold text-slate-900 dark:text-brand-50">Total</span>
                                <span className="text-2xl font-extrabold text-slate-900 dark:text-brand-50">GHS {grandTotal.toFixed(2)}</span>
                            </div>

                            <button
                                onClick={handleAction}
                                disabled={paying || (deliveryMethod === 'delivery' && locating)}
                                className="w-full py-3 rounded-xl bg-gradient-to-r from-brand-600 to-accent-500 dark:from-brand-600 dark:to-brand-400 hover:opacity-90 text-white dark:text-ink-900 font-semibold text-sm transition shadow-sm disabled:opacity-60"
                            >
                                {paying ? 'Redirecting to payment…' : (deliveryMethod === 'pickup' ? 'Chat with Seller(s)' : `Pay · GHS ${grandTotal.toFixed(2)}`)}
                            </button>
                            {!user && (
                                <p className="text-xs text-slate-400 dark:text-brand-200/50 text-center mt-3">
                ...
                <div className="fixed inset-0 z-[110] flex items-end sm:items-center justify-center">
                    ...
                    <div className="relative w-full sm:max-w-md bg-white dark:bg-ink-800 rounded-t-2xl sm:rounded-2xl shadow-2xl p-5 sm:p-6">
                        <div className="flex items-center justify-between mb-1">
                            ...
                            <button
                                onClick={() => !sendingBroadcast && setShowBroadcastModal(false)}
                                className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-ink-700 text-slate-400 dark:text-brand-200/50 transition"
                            >
                                <X size={18} />
                            </button>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-1.5 mb-4">
                            {sellersForChat.map((s) => (
                                <span
                                    key={s.sellerId}
                                    className="text-xs font-semibold px-2.5 py-1 rounded-full bg-slate-100 dark:bg-ink-700 text-slate-600 dark:text-brand-200"
                                >
                                    {s.sellerName}
                                </span>
                            ))}
                        ...
                        <button
                            ...
                            className="w-full mt-4 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-brand-600 dark:bg-brand-500 hover:bg-brand-700 dark:hover:bg-brand-400 text-white dark:text-ink-900 font-semibold text-sm transition disabled:opacity-60"
                        >
                            {sendingBroadcast ? (
                                <>
...
function CartHeader({ count }) {
    return (
        <section className="relative overflow-hidden shrink-0">
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-brand-900/50 via-brand-800/35 to-accent-600/25 dark:from-ink-900/80 dark:via-ink-900/60 dark:to-brand-900/35" />
