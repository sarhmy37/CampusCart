...
export default function Store({ id: idProp, embedded = false }) {
    ...
    if (notFound || !seller) {
        return (
            <div className="max-w-md mx-auto px-4 py-20 text-center">
                <p className="text-lg font-bold text-slate-800 dark:text-brand-100">Store not found</p>
                <p className="text-sm text-slate-400 dark:text-brand-200/50 mt-2">
                    This seller doesn't exist or their account is no longer active.
                </p>
                <Link
                    to="/browse"
                    className="inline-flex items-center gap-1.5 mt-6 px-5 py-2.5 rounded-xl bg-brand-600 dark:bg-brand-500 hover:bg-brand-700 dark:hover:bg-brand-400 text-white dark:text-ink-900 font-semibold text-sm transition"
                >
                    Browse listings →
                </Link>
            </div>
    ...
    return (
        <div>
            {/* HEADER */}
            <section className="relative overflow-hidden bg-gradient-to-br from-brand-900 via-brand-700 to-accent-600 dark:from-ink-900 dark:via-ink-800 dark:to-brand-900">
                {seller.avatar_url && (
                    <div
                ...
                <div className="absolute inset-0 bg-gradient-to-br from-brand-900 via-brand-700 to-accent-600 dark:from-ink-900 dark:via-ink-800 dark:to-brand-900 opacity-25 pointer-events-none" />          
