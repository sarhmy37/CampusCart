    ...
    return (
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10 bg-white dark:bg-ink-900 min-h-screen">
            <button
                ...
                className="inline-flex items-center gap-2 text-sm font-semibold text-slate-500 dark:text-brand-200/60 hover:text-slate-700 dark:hover:text-brand-100 transition mb-6"
            >
                <ArrowLeft size={16} /> Back
            </button>

            <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-brand-50">Terms of Service</h1>
            <p className="text-sm text-slate-400 dark:text-brand-200/50 mt-1">Last updated: 2026</p>

            <div className="mt-8 space-y-6 text-sm text-slate-600 dark:text-brand-200/70 leading-relaxed">
...
function Section({ title, children }) {
    return (
        <div>
            <h2 className="font-bold text-slate-800 dark:text-brand-100 mb-1.5">{title}</h2>
            <p>{children}</p>
        </div>
    );
}