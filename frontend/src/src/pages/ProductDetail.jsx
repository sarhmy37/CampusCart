...
export default function ProductDetail() {
    ...
    if (!product) {
        return <div className="max-w-5xl mx-auto px-4 py-24 text-center text-slate-400 dark:text-brand-200/50 dark:bg-ink-900 min-h-screen">Loading…</div>;
    }
    ...
    const SellerCard = () => (
        <GlassCard>
            <p className="text-xs font-semibold text-slate-400 dark:text-brand-200/50 uppercase tracking-wide mb-3">Sold by</p>
            <div className="flex items-center gap-3">
                <div className="relative shrink-0">
                    {product.seller_avatar ? (
                        <img
                            ...
                            alt={sellerName}
                            className="w-11 h-11 rounded-full object-cover ring-2 ring-white/50 dark:ring-brand-500/30"
                        />
                    ) : (
                        <div className="w-11 h-11 rounded-full bg-gradient-to-br from-brand-400 to-accent-500 dark:from-brand-400 dark:to-brand-600 text-white flex items-center justify-center font-bold text-sm shadow-lg">
                            {sellerInitial}
                        </div>
                    )}
                ...
                <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5">
                        <p className="font-bold text-slate-900 dark:text-brand-50 truncate">{sellerName}</p>
                    ...
                    {product.seller_verified ? (
                        ...
                        <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-amber-600 dark:text-brand-400 mt-0.5">
                            Not yet verified
                        </span>
                    )}
                    {!isDemo && (
                        <p className="text-[11px] text-slate-400 dark:text-brand-200/50 mt-0.5">
            ...
            <div className="mt-4 pt-4 border-t border-slate-200/50 dark:border-white/10 space-y-2.5">
                {product.seller_school && (
                    <div className="flex items-center gap-2 text-sm">
                        <MapPin size={14} className="text-slate-400 dark:text-brand-300/50 shrink-0" />
                        <span className="text-slate-600 dark:text-brand-100/80">{product.seller_school}</span>
                    </div>
                )}
                {product.seller_meeting_place && (
                    <div className="flex items-center gap-2 text-sm">
                        <Tag size={14} className="text-slate-400 dark:text-brand-300/50 shrink-0" />
                        <span className="text-slate-600 dark:text-brand-100/80">Meets at: {product.seller_meeting_place}</span>
                    </div>
                )}
                {(product.seller_whatsapp || product.whatsapp) && (
                    <a
                        ...
                        className={`flex items-center gap-2 text-sm transition ${
                            isOwner
                                ? 'text-slate-400 dark:text-brand-200/40 cursor-not-allowed'
        ...
        <GlassCard>
            <div className="space-y-3">
                <div className="flex items-center gap-3">
                    ...
                    <p className="text-sm text-slate-600 dark:text-brand-100/80">Email-verified student seller</p>
                </div>
                <div className="flex items-center gap-3">
                    <Truck size={18} className="text-brand-600 dark:text-brand-400 shrink-0" />
                    <p className="text-sm text-slate-600 dark:text-brand-100/80">Meet up on campus, or arrange delivery with the seller</p>
                </div>
                <div className="flex items-center gap-3">
                    <Clock size={18} className="text-brand-600 dark:text-brand-400 shrink-0" />
                    <p className="text-sm text-slate-600 dark:text-brand-100/80">Usually responds within a few hours</p>
                </div>
            </div>
        </GlassCard>
    );
    ...
    const ActionCard = () => (
        <GlassCard>
            <span className="inline-block px-2.5 py-0.5 rounded-full bg-brand-50 dark:bg-brand-900 text-brand-700 dark:text-brand-400 text-xs font-semibold capitalize">
                {product.condition}
            </span>
            <h1 className="text-xl font-extrabold text-slate-900 dark:text-brand-50 mt-2 leading-snug">{product.title}</h1>
            {reviews?.avg_rating && (
                <div className="flex items-center gap-1.5 mt-1 text-sm">
                    <Star size={14} className="fill-amber-400 text-amber-400" />
                    <span className="font-semibold text-slate-700 dark:text-brand-100">{reviews.avg_rating}</span>
                    <span className="text-slate-400 dark:text-brand-200/50">({reviews.total} reviews)</span>
                </div>
            )}
                ...
                {discountPercent !== null ? (
                    <>
                        <div className="flex items-center gap-2">
                            <span className="text-sm text-slate-400 dark:text-brand-200/50 line-through">
                        ...
                        <p className="text-3xl font-extrabold text-brand-700 dark:text-brand-400">
            ...
            <div className="flex items-center gap-3 mt-4">
                <span className="text-sm font-semibold text-slate-700 dark:text-brand-100">Qty</span>
                <div className="flex items-center gap-3 border border-slate-200/50 dark:border-white/10 bg-white/50 dark:bg-ink-700/50 backdrop-blur-sm rounded-lg px-3 py-1.5">
                    <button
                        ...
                        className="text-slate-500 dark:text-brand-200/60 hover:text-slate-800 dark:hover:text-brand-100 disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                        <Minus size={14} />
                    </button>
                    <span className="text-sm w-4 text-center font-semibold text-slate-800 dark:text-brand-100">{qty}</span>
                    <button
            ...
            <div className="mt-4 flex flex-col gap-2.5">
                <button
                    ...
                    className={`w-full flex items-center justify-center gap-2 py-3 rounded-xl font-semibold text-sm transition-all duration-200 ${
                        isOutOfStock || isOwner
                            ? 'bg-slate-200/50 dark:bg-ink-700/50 text-slate-400 dark:text-brand-200/30 cursor-not-allowed backdrop-blur-sm'
                            : 'bg-gradient-to-r from-brand-600 to-accent-500 dark:from-brand-500 dark:to-brand-400 hover:scale-[1.02] active:scale-[0.98] text-white dark:text-ink-900 shadow-lg shadow-brand-500/25 dark:shadow-brand-500/25'
                    }`}
                >
                ...
                <button
                    ...
                    className={`w-full flex items-center justify-center gap-2 py-3 rounded-xl border font-semibold text-sm transition-all duration-200 ${
                        isOutOfStock || isOwner
                            ? 'border-slate-200/50 dark:border-white/10 text-slate-400 dark:text-brand-200/30 cursor-not-allowed backdrop-blur-sm'
                            : 'border-slate-200/50 dark:border-white/10 bg-white/50 dark:bg-ink-700/50 backdrop-blur-sm hover:bg-white/80 dark:hover:bg-ink-600/80 hover:border-brand-400 dark:hover:border-brand-500 text-slate-700 dark:text-brand-100'
                    }`}
                >
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 bg-gradient-to-b from-slate-50 to-white dark:from-ink-900 dark:to-ink-950 min-h-screen">
            <Link to="/browse" className="inline-flex items-center gap-1 text-sm text-slate-500 dark:text-brand-200/60 hover:text-brand-600 dark:hover:text-brand-400 mb-6 transition">
            ...
            <div className="grid lg:grid-cols-5 gap-8">
                ...
                <div className="lg:col-span-3">
                        ...
                        <div className="flex gap-2 mt-3">
                            {slides.map((slide, i) => (
                                <button
                                    key={i}
                                    onClick={() => setActiveImg(i)}
                                    className={`relative w-16 h-16 rounded-lg overflow-hidden border-2 ${activeImg === i ? 'border-brand-500 dark:border-brand-500' : 'border-transparent'}`}
                                >
                                    {slide.video_url ? (
                                        <>
                    ...
                    <div className="mt-6 bg-white/70 dark:bg-ink-800/70 backdrop-blur-xl border border-white/30 dark:border-white/10 rounded-2xl p-5 shadow-[0_8px_32px_rgba(0,0,0,0.08)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.3)]">
                        <h2 className="font-bold text-slate-900 dark:text-brand-50 mb-2">Description</h2>
                        <p className="text-slate-600 dark:text-brand-100/80 text-sm leading-relaxed whitespace-pre-line">
                    ...
                    <div className="mt-4 bg-white/70 dark:bg-ink-800/70 backdrop-blur-xl border border-white/30 dark:border-white/10 rounded-2xl p-5 shadow-[0_8px_32px_rgba(0,0,0,0.08)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.3)]">
                        <h2 className="font-bold text-slate-900 dark:text-brand-50 mb-3">Details</h2>
                        <dl className="grid grid-cols-2 gap-y-3 text-sm">
                            <dt className="text-slate-400 dark:text-brand-200/50">Condition</dt>
                            <dd className="text-slate-800 dark:text-brand-100 font-medium capitalize">{product.condition}</dd>
                            {relatedCategory && (
                                <>
                                    <dt className="text-slate-400 dark:text-brand-200/50">Category</dt>
                                    <dd className="text-slate-800 dark:text-brand-100 font-medium">{relatedCategory}</dd>
                                </>
                            )}
                            <dt className="text-slate-400 dark:text-brand-200/50">Available</dt>
                            <dd className="text-slate-800 dark:text-brand-100 font-medium">{stock} in stock</dd>
                            {posted && (
                                <>
                                    <dt className="text-slate-400 dark:text-brand-200/50">Posted</dt>
                                    <dd className="text-slate-800 dark:text-brand-100 font-medium">{posted}</dd>
                                </>
                            )}
                        </dl>
                    </div>
                    ...
                    {!isDemo && (
                        <div className="lg:hidden mt-4">
                            <button
                                ...
                                className="w-full flex items-center justify-center gap-1.5 text-xs font-semibold text-slate-400 dark:text-brand-200/40 hover:text-red-500 dark:hover:text-red-400 transition py-2"
                            >
            ...
            {similar.length > 0 && (
                <div className="mt-12">
                    <h2 className="font-bold text-slate-900 dark:text-brand-50 text-lg mb-4">Similar listings</h2>
