...
export default function OrderConfirmation() {
    ...
    if (loading) {
        return (
            <div className="max-w-lg mx-auto px-4 py-24 text-center text-slate-400 dark:text-brand-200/50">
                Loading your order…
            </div>
        );
    }
    ...
    if (error || !order) {
        return (
            <div className="max-w-lg mx-auto px-4 py-24 text-center">
                <p className="text-slate-500 dark:text-brand-200/60">We couldn't find that order.</p>
                <Link to="/dashboard" className="inline-block mt-4 text-brand-600 dark:text-brand-400 font-semibold text-sm">
    ...
    return (
        <div className="max-w-lg mx-auto px-4 sm:px-6 py-12 dark:bg-ink-900 min-h-screen">
            <Link to="/dashboard" className="inline-flex items-center gap-1 text-sm text-slate-500 dark:text-brand-300/60 hover:text-brand-600 dark:hover:text-brand-400 mb-6">
            ...
            <div className="bg-white dark:bg-ink-800 border border-slate-200 dark:border-ink-600 rounded-2xl p-8 text-center">
                ...
                <h1 className="text-xl font-extrabold text-slate-900 dark:text-brand-50">{config.label}</h1>
                <p className="text-sm text-slate-400 dark:text-brand-200/50 mt-1">Order #{order.id.slice(0, 8)}</p>

                <div className="mt-6 text-left space-y-3">
                    {order.items.map((item) => (
                        <div key={item.id} className="flex justify-between text-sm">
                            <span className="text-slate-600 dark:text-brand-200/80">{item.title} × {item.quantity}</span>
                ...
                <div className="border-t border-slate-100 dark:border-ink-600 mt-5 pt-5 space-y-2 text-left">
                    <div className="flex justify-between text-sm text-slate-500 dark:text-brand-300/60">
                        <span>Subtotal</span>
                    ...
                    <div className="flex justify-between font-bold text-slate-900 dark:text-brand-50 text-base pt-2">
                        <span>Total</span>
                        <span>GHS {parseFloat(order.total_amount).toFixed(2)}</span>
                    </div>
                </div>

                <div className="mt-6 flex items-center justify-center gap-2 text-xs text-slate-400 dark:text-brand-300/60">
            ...
            <Link
                to="/browse"
                className="block text-center mt-6 text-sm font-semibold text-brand-600 dark:text-brand-400 hover:text-brand-700 dark:hover:text-brand-300"
            >
                Continue browsing →
            </Link>
        </div>
