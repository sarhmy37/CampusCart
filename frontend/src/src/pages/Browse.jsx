...
export default function Browse() {
    ...
    return (
        <div className="relative min-h-screen">
            {/* HEADER STRIP */}
            <section className="sticky top-14 sm:top-16 z-30 relative overflow-hidden bg-gradient-to-br from-brand-900 via-brand-700 to-accent-600 dark:from-ink-900 dark:via-ink-800 dark:to-brand-900">
                <div className="absolute inset-0" style={{ opacity: imageOpacity }}>
                    <HeroSlideshow images={BROWSE_HEADER_IMAGES} />
                    <div className="absolute inset-0 bg-gradient-to-br from-brand-900/70 via-brand-800/50 to-accent-600/40 dark:from-ink-900/85 dark:via-ink-900/60 dark:to-brand-900/30" />
                </div>
                <div className="absolute -right-16 -top-20 w-72 h-72 bg-white/10 rounded-full blur-2xl" style={{ opacity: imageOpacity }} />
                <div className="absolute left-1/3 -bottom-20 w-56 h-56 bg-brand-300/20 dark:bg-brand-300/10 rounded-full blur-3xl" style={{ opacity: imageOpacity }} />
                                ...
                                <button
                                    ...
                                    className={`text-sm font-semibold px-3.5 py-1.5 rounded-full border transition backdrop-blur ${
                                        priceRange?.label === r.label
                                            ? 'bg-white text-brand-700 dark:bg-brand-500 dark:text-ink-900 border-white dark:border-brand-500'
                            ...
                            <div key="categories" className="relative inline-flex items-center">
                                <select
                                    ...
                                    className={`appearance-none inline-flex items-center text-sm pl-8 pr-6 py-1.5 rounded-full border transition-all cursor-pointer ${
                                        active
                                            ? 'bg-brand-600 dark:bg-brand-600 text-white dark:text-ink-900 border-brand-600 dark:border-brand-600 font-bold'
                                            : 'bg-white dark:bg-ink-800 text-slate-700 dark:text-brand-200 border-slate-200 dark:border-ink-600 hover:bg-slate-50 dark:hover:bg-ink-700 font-semibold'
                                    }`}
                                >
                                ...
                                <Icon className={`pointer-events-none absolute left-2.5 w-4 h-4 ${active ? 'text-white dark:text-ink-900' : 'text-slate-500 dark:text-brand-300/60'}`} />
                            </div>
                        );
                    }
            ...
            <section className="relative overflow-hidden bg-slate-100 dark:from-ink-900 dark:via-ink-950 dark:to-ink-900 dark:bg-gradient-to-b">
                ...
                <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 pb-32 sm:pb-10">
                    <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap mb-2">
                        {itemCategory && (
                            <span className="inline-flex items-center gap-1.5 bg-brand-600 dark:bg-brand-600 text-white dark:text-ink-900 px-3 py-1 rounded-full text-xs font-semibold">
                                {itemCategory}
                        ...
                        {school && filterType !== 'nearby' && (
                            <span className="inline-flex items-center gap-1.5 bg-slate-700 dark:bg-ink-700 text-white dark:text-brand-200 px-3 py-1 rounded-full text-xs font-semibold">
                                📍 {school}
                        ...
                        {priceRange && (
                            <span className="inline-flex items-center gap-1.5 bg-slate-800 dark:bg-brand-900 text-white dark:text-brand-100 px-3 py-1 rounded-full text-xs font-semibold">
                                {priceRange.label}
                    ...
                    {loading ? (
                        ...
                        <div className="text-center py-20 text-slate-400 dark:text-brand-200/40">
                        ...
                        <>
                            {isDemo && (
                                <p className="text-sm text-slate-400 dark:text-brand-200/40 mb-4">No live listings yet — here's a preview of how they'll look:</p>
                            )}
                            <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
                    ...
                    {tabs.map((tab) => {
                        ...
                        return (
                            ...
                            >
                                    ...
                                    <span
                                        className={`text-[9.5px] leading-none truncate max-w-full px-0.5 transition-all duration-300 ${
                                            active
                                                ...
                                                : 'font-medium text-slate-500 dark:text-brand-200/50'
                                        }`}
                                    >
                                        {label}
                                    </span>
                                ...
                                {active && (
                                    <span className="absolute left-1/2 -translate-x-1/2 -bottom-0.5 h-[3.5px] w-6 rounded-full bg-brand-600 dark:bg-brand-500 transition-all duration-300" />
                                )}
                            </button>
                        );
                    })}
        ...
        <div className="sm:hidden fixed inset-0 z-50">
            ...
            >
                ...
                <div className="flex items-center justify-between px-5 pb-3 shrink-0">
                    <h3 className="text-base font-bold text-slate-900 dark:text-brand-100">{title}</h3>
                    <button
                        onClick={onClose}
                        className="p-1.5 rounded-full bg-slate-100 dark:bg-ink-700 text-slate-500 dark:text-brand-200/60"
                    >
                        <X size={16} />
                    </button>
                </div>
                            ...
                            <button
                                ...
                                className={`w-full flex items-center justify-between text-left px-4 py-3 rounded-xl text-sm transition ${
                                    isSelected
                                        ...
                                        : 'text-slate-700 dark:text-brand-100 font-medium hover:bg-slate-50 dark:hover:bg-ink-700'
                                }`}
                            >
                                {opt.label}
