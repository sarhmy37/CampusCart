...
export default function Admin() {
    ...
    return (
        <div className="bg-white dark:bg-ink-900 min-h-screen">
            <section className="relative overflow-hidden bg-gradient-to-br from-brand-900 via-brand-800 to-accent-600 dark:from-ink-900 dark:via-ink-800 dark:to-brand-900">
            ...
            return (
                <button
                    ...
                    className={`flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-sm font-semibold transition whitespace-nowrap ${
                        tab === t
                            ? 'bg-white dark:bg-ink-700 shadow-sm text-brand-700 dark:text-brand-400'
                            : 'text-slate-500 dark:text-brand-200/50 hover:text-slate-700 dark:hover:text-brand-300'
                    }`}
                >
                    {Icon && <Icon size={15} />}
                    {label}
            ...
            <div className="relative transform transition-all duration-300 hover:scale-[1.002]">
                                ...
                                <button
                                    ...
                                    className={`tab-item relative shrink-0 h-full flex flex-col items-center justify-center gap-0.5 px-4 text-xs transition-all duration-200 ${
                                        isActive
                                            ...
                                            : 'text-slate-500 dark:text-brand-200/50 font-semibold hover:text-slate-700 dark:hover:text-brand-300'
                                    }`}
                                    style={{ minWidth: 70 }}
                                >
                                    {Icon && <Icon size={16} className={isActive ? 'text-brand-600 dark:text-brand-400' : 'text-current'} />}
                                    <span className="whitespace-nowrap">{label}</span>
                                    {isActive && (
                                        <span
                                            className="absolute -bottom-[1px] left-1/2 -translate-x-1/2 h-1 bg-brand-600 dark:bg-brand-500 rounded-full transition-all duration-300"
                ...
                {showLeftChevron && (
                    <div className="pointer-events-none absolute inset-y-0 left-0 w-8 rounded-l-xl bg-gradient-to-r from-white/95 dark:from-ink-800/95 to-transparent flex items-center">
                        <ChevronLeft size={14} className="text-slate-400 dark:text-brand-300/40 ml-1" />
                    </div>
                )}
                {showRightChevron && (
                    <div className="pointer-events-none absolute inset-y-0 right-0 w-8 rounded-r-xl bg-gradient-to-l from-white/95 dark:from-ink-800/95 to-transparent flex items-center justify-end">
                        <ChevronRight size={14} className="text-slate-400 dark:text-brand-300/40 mr-1" />
                    </div>
                )}
            </div>
        </div>
    ...
    return (
        <div className="max-w-4xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 mb-4">
                <div className="flex-1 flex items-center bg-white dark:bg-ink-800 border border-slate-200 dark:border-ink-600 rounded-xl overflow-hidden focus-within:border-brand-500 dark:focus-within:border-brand-500 focus-within:ring-2 focus-within:ring-brand-100 dark:focus-within:ring-brand-900 transition">
                    <Search size={18} className="ml-3 text-slate-400 dark:text-brand-300/40 shrink-0" />
                    <input
                        type="text"
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        placeholder="Search users by name, email, or school..."
                        className="w-full px-2 py-2.5 bg-transparent text-sm text-slate-800 dark:text-brand-50 placeholder:text-slate-400 dark:placeholder:text-brand-300/40 focus:outline-none"
                    />
                    {searchQuery && (
                        <button
                            onClick={() => setSearchQuery('')}
                            className="px-2 text-slate-400 dark:text-brand-300/40 hover:text-slate-600 dark:hover:text-brand-200"
                        >
                            <X size={16} />
                        </button>
                    )}
                ...
                <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition whitespace-nowrap ${
                        showFilters || activeFilterCount > 0
                            ...
                            : 'bg-slate-100 dark:bg-ink-800 text-slate-600 dark:text-brand-200/60 hover:bg-slate-200 dark:hover:bg-ink-700'
                    }`}
                >
                    <Filter size={16} />
                    Filters
                    {activeFilterCount > 0 && (
                        <span className="ml-1 px-2 py-0.5 rounded-full bg-brand-600 dark:bg-brand-500 text-white dark:text-ink-900 text-xs">
                            {activeFilterCount}
                        </span>
                    )}
                </button>
                {activeFilterCount > 0 && (
                    <button
                        ...
                        className="text-xs text-slate-400 dark:text-brand-200/50 hover:text-red-500 dark:hover:text-red-400 transition flex items-center gap-1 whitespace-nowrap"
                    >
            ...
            {showFilters && (
                <div className="bg-white dark:bg-ink-800 border border-slate-200 dark:border-ink-600 rounded-2xl p-4 mb-4 shadow-sm">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {/* Search Field */}
                        <div>
                            <label className="text-xs font-semibold text-slate-500 dark:text-brand-300/60 uppercase tracking-wide flex items-center gap-1.5 mb-1.5">
                                <Search size={14} /> Search In
                            </label>
                            <select
                                value={searchField}
                                onChange={(e) => setSearchField(e.target.value)}
                                className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-ink-600 dark:bg-ink-700 dark:text-brand-50 text-sm focus:border-brand-500 dark:focus:border-brand-500 focus:outline-none transition"
                            >
            ...
            <div className="flex items-center justify-between mb-3">
                <p className="text-sm text-slate-400 dark:text-brand-200/50">
            ...
            {filteredUsers.length === 0 ? (
                <div className="text-center py-10 text-slate-400 dark:text-brand-200/40">No users found matching your filters.</div>
            ) : (
                    ...
                    {filteredUsers.map((u) => (
                        <div key={u.id} className="flex items-center justify-between gap-3 bg-white dark:bg-ink-800 border border-slate-200 dark:border-ink-600 rounded-2xl p-4 flex-wrap">
                            <div className="min-w-0">
                                <p className="text-sm font-semibold text-slate-800 dark:text-brand-100 truncate">{u.name}</p>
                                <p className="text-xs text-slate-400 dark:text-brand-200/50 truncate">{u.university_email}</p>
                                {u.school && (
                                    <p className="text-xs text-slate-400 dark:text-brand-200/50 truncate">🏫 {u.school}</p>
                                )}
    ...
    return (
        <div className="fixed inset-0 z-[70] flex items-center justify-center px-4">
            ...
            <div className="relative bg-white dark:bg-ink-800 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6 border border-slate-200 dark:border-ink-600">
                ...
                <h2 className="text-xl font-extrabold text-slate-900 dark:text-brand-50">Investigate Account</h2>
                <p className="text-sm text-slate-500 dark:text-brand-200/50 mt-1">{user.university_email}</p>

                <div className="mt-4 grid grid-cols-2 gap-3 text-sm bg-slate-50 dark:bg-ink-700 rounded-xl p-4">
                    <div><span className="text-slate-400 dark:text-brand-200/50">Name</span><br /><span className="font-semibold text-slate-800 dark:text-brand-100">{user.name}</span></div>
                    <div><span className="text-slate-400 dark:text-brand-200/50">Account Type</span><br /><span className="font-semibold text-slate-800 dark:text-brand-100 capitalize">{user.account_type}</span></div>
                    <div><span className="text-slate-400 dark:text-brand-200/50">Role</span><br /><span className="font-semibold text-slate-800 dark:text-brand-100 capitalize">{user.role}</span></div>
                    <div><span className="text-slate-400 dark:text-brand-200/50">Verified</span><br /><span className={`font-semibold ${user.verified ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>{user.verified ? '✅ Yes' : '❌ No'}</span></div>
                    <div><span className="text-slate-400 dark:text-brand-200/50">Banned</span><br /><span className={`font-semibold ${user.banned ? 'text-red-600 dark:text-red-400' : 'text-emerald-600 dark:text-emerald-400'}`}>{user.banned ? '🚫 Yes' : '✅ No'}</span></div>
                    <div><span className="text-slate-400 dark:text-brand-200/50">Joined</span><br /><span className="font-semibold text-slate-800 dark:text-brand-100">{new Date(user.created_at).toLocaleDateString()}</span></div>
                </div>

                <div className="mt-4">
                    <h3 className="font-bold text-slate-800 dark:text-brand-100 mb-2">Listings ({listings.length})</h3>
                    {loading ? (
                        <div className="text-center py-4 text-slate-400 dark:text-brand-200/50">Loading...</div>
                    ) : listings.length === 0 ? (
                        <p className="text-sm text-slate-400 dark:text-brand-200/50">No listings found.</p>
                    ) : (
                        <div className="space-y-1.5 max-h-40 overflow-y-auto">
                            {listings.map((p) => (
                                <div key={p.id} className="flex items-center justify-between text-sm bg-slate-50 dark:bg-ink-700 rounded-lg px-3 py-2">
                                    <span className="text-slate-700 dark:text-brand-100 truncate">{p.title}</span>
                                    <span className="text-slate-400 dark:text-brand-200/50">GHS {parseFloat(p.price).toFixed(2)}</span>
                                </div>
                            ))}
                        </div>
                    )}
                ...
                <div className="mt-4">
                    <h3 className="font-bold text-slate-800 dark:text-brand-100 mb-2">Orders ({orders.length})</h3>
                    {loading ? (
                        <div className="text-center py-4 text-slate-400 dark:text-brand-200/50">Loading...</div>
                    ) : orders.length === 0 ? (
                        <p className="text-sm text-slate-400 dark:text-brand-200/50">No orders found.</p>
                    ) : (
                        <div className="space-y-1.5 max-h-40 overflow-y-auto">
                            {orders.map((o) => (
                                <div key={o.id} className="flex items-center justify-between text-sm bg-slate-50 dark:bg-ink-700 rounded-lg px-3 py-2">
                                    <span className="text-slate-700 dark:text-brand-100">Order #{o.id}</span>
                                    <span className={`text-xs font-semibold px-2 py-0.5 rounded-full capitalize ${
                                        o.status === 'completed' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400' :
                                        o.status === 'pending' ? 'bg-amber-50 text-amber-700 dark:bg-brand-900/40 dark:text-brand-400' :
                                        'bg-slate-100 text-slate-500 dark:bg-ink-700 dark:text-brand-200/50'
                                    }`}>{o.status}</span>
                                </div>
                            ))}
                        </div>
    ...
    return (
        <div className="max-w-4xl mx-auto">
            <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                <div className="flex items-center gap-2">
                    ...
                    {activeFilterCount > 0 && (
                        <button
                            onClick={clearAllFilters}
                            className="text-xs text-slate-400 dark:text-brand-200/50 hover:text-red-500 dark:hover:text-red-400 transition flex items-center gap-1"
                        >
                            <XClose size={14} /> Clear all
                        </button>
                    )}
            ...
            {filteredListings.length === 0 ? (
                <div className="text-center py-10 text-slate-400 dark:text-brand-200/40">
                    No listings found matching your filters.
                </div>
            ) : (
                <div className="space-y-2">
                    {filteredListings.map((p) => (
                        <div key={p.id} className="flex items-center gap-4 bg-white dark:bg-ink-800 border border-slate-200 dark:border-ink-600 rounded-2xl p-3 hover:border-brand-300 dark:hover:border-brand-500 transition">
                            ...
                            <div className="flex-1 min-w-0">
                                <p className="font-semibold text-slate-800 dark:text-brand-100 text-sm truncate">{p.title}</p>
                                <div className="flex items-center gap-2 text-xs text-slate-400 dark:text-brand-200/50 flex-wrap">
                                    <span>{p.seller_name}</span>
                                    <span>·</span>
                                    <span className="font-medium text-slate-600 dark:text-brand-200/70">GHS {parseFloat(p.price).toFixed(2)}</span>
                                    <span>·</span>
                                    <span className="font-medium text-slate-600 dark:text-brand-200/70">Stock: {p.stock}</span>
                                    {p.category && (
                                        <>
                                            <span>·</span>
                            ...
                            <button 
                                onClick={() => remove(p.id)} 
                                className="text-slate-300 dark:text-brand-300/40 hover:text-red-500 p-1.5 transition shrink-0"
        ...
        <div className="max-w-3xl mx-auto">
            <form onSubmit={runSearch} className="flex gap-2 mb-4">
                <input
                    ...
                    className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 dark:border-ink-600 dark:bg-ink-800 dark:text-brand-50 focus:border-brand-500 dark:focus:border-brand-500 focus:ring-2 focus:ring-brand-100 dark:focus:ring-brand-900 focus:outline-none text-sm transition"
                />
                <button
                    type="submit"
                    className="px-4 py-2.5 rounded-xl bg-brand-600 dark:bg-brand-500 hover:bg-brand-700 dark:hover:bg-brand-400 text-white dark:text-ink-900 text-sm font-semibold transition"
                >
                    Search
                </button>
            </form>
                ...
                <div className="space-y-2">
                    {orders.map((o) => (
                        <button
                            ...
                            className="w-full flex items-center justify-between gap-3 bg-white dark:bg-ink-800 border border-slate-200 dark:border-ink-600 hover:border-brand-300 dark:hover:border-brand-500 rounded-2xl p-4 text-left transition"
                        >
                            <div className="min-w-0">
                                <p className="text-sm font-semibold text-slate-800 dark:text-brand-100 font-mono truncate">
                                    #{o.id}
                                </p>
                                <p className="text-xs text-slate-400 dark:text-brand-200/50 truncate">
                                    {o.buyer_name} · {o.delivery_method} · {new Date(o.created_at).toLocaleDateString()}
                                </p>
                            </div>
                            <div className="text-right shrink-0">
                                <p className="text-sm font-bold text-slate-900 dark:text-brand-50">GHS {parseFloat(o.total_amount).toFixed(2)}</p>
                                <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-brand-50 dark:bg-brand-900/40 text-brand-700 dark:text-brand-300 capitalize">{o.status}</span>
                            </div>
                        </button>
                    ))}
                </div>
                ...
                {loading || !order ? (
                    ...
                    <>
                        <div className="flex items-start justify-between gap-3 mb-5">
                            <div>
                                <p className="text-xs text-slate-400 dark:text-brand-200/50 font-mono">#{order.id}</p>
                                <h3 className="text-lg font-extrabold text-slate-900 dark:text-brand-50 mt-1">Order details</h3>
                            </div>
                            <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-brand-50 dark:bg-brand-900/40 text-brand-700 dark:text-brand-300 capitalize shrink-0">
                                {order.status}
                            </span>
                        </div>
                        ...
                        <div className="grid sm:grid-cols-2 gap-4 mb-5">
                            <DetailBlock title="Buyer">
                                <p className="font-semibold text-slate-800 dark:text-brand-100">{order.buyer_name}</p>
                            ...
                            <DetailBlock title="Order timing">
                                ...
                                <p className="mt-1">Delivery method: <span className="capitalize font-semibold text-slate-700 dark:text-brand-100">{order.delivery_method}</span></p>
                                {order.payment_reference && <p className="font-mono text-[11px] mt-1">Ref: {order.payment_reference}</p>}
                            </DetailBlock>
                        </div>

                        <div className="mb-5">
                            <p className="text-xs font-semibold text-slate-500 dark:text-brand-300/60 uppercase tracking-wide mb-2">Items ({order.items.length})</p>
                            <div className="space-y-2">
                                {order.items.map((item) => (
                                    <div key={item.id} className="bg-slate-50 dark:bg-ink-700 rounded-xl p-3">
                                        <div className="flex items-start justify-between gap-3">
                                            <div className="min-w-0">
                                                <p className="text-sm font-semibold text-slate-800 dark:text-brand-100">{item.title}</p>
                                                <p className="text-xs text-slate-500 dark:text-brand-200/60 mt-0.5">
                                                    Qty {item.quantity} · GHS {parseFloat(item.price_at_purchase).toFixed(2)} each
                                                </p>
                                            </div>
                                            <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-white dark:bg-ink-800 text-slate-600 dark:text-brand-200/70 capitalize shrink-0">
                                                {item.status}
                                            </span>
                                        </div>
                                        <div className="grid grid-cols-3 gap-2 mt-2.5 pt-2.5 border-t border-slate-200 dark:border-ink-600 text-xs">
                                            <div>
                                                <p className="text-slate-400 dark:text-brand-200/50">Seller</p>
                                                <p className="font-semibold text-slate-700 dark:text-brand-100 truncate">{item.seller_name}</p>
                                                <p className="text-slate-400 dark:text-brand-200/40 truncate">{item.seller_email}</p>
                                            </div>
                                            <div>
                                                <p className="text-slate-400 dark:text-brand-200/50">Platform fee</p>
                                                <p className="font-semibold text-slate-700 dark:text-brand-100">GHS {parseFloat(item.platform_fee).toFixed(2)}</p>
                                            </div>
                                            <div>
                                                <p className="text-slate-400 dark:text-brand-200/50">Seller earnings</p>
                                                <p className="font-semibold text-brand-700 dark:text-brand-400">GHS {parseFloat(item.seller_earnings).toFixed(2)}</p>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                        ...
                        <div className="border-t border-slate-100 dark:border-ink-600 pt-4 space-y-1.5 text-sm">
                            <div className="flex justify-between text-slate-500 dark:text-brand-200/60">
                                <span>Subtotal</span>
                            ...
                            <div className="flex justify-between font-bold text-slate-900 dark:text-brand-50 text-base pt-1.5 border-t border-slate-100 dark:border-ink-600 mt-1.5">
                        ...
                        <button
                            onClick={onClose}
                            className="w-full mt-6 py-2.5 rounded-xl border border-slate-200 dark:border-ink-600 text-slate-600 dark:text-brand-200 text-sm font-semibold hover:bg-slate-50 dark:hover:bg-ink-700 transition"
                        >
                            Close
                        </button>
                    </>
...
function DetailBlock({ title, children }) {
    return (
        <div className="bg-slate-50 dark:bg-ink-700 rounded-xl p-3.5">
            <p className="text-xs font-semibold text-slate-500 dark:text-brand-300/60 uppercase tracking-wide mb-2">{title}</p>
            <div className="text-sm text-slate-600 dark:text-brand-200/70 space-y-0.5">{children}</div>
        </div>
    );
}
        ...
        <div className="max-w-3xl mx-auto">
                ...
                {REPORT_STATUS_FILTERS.map((s) => (
                    <button
                        ...
                        className={`px-3.5 py-1.5 rounded-lg text-xs font-semibold capitalize transition ${
                            statusFilter === s
                                ? 'bg-white dark:bg-ink-700 shadow-sm text-brand-700 dark:text-brand-400'
                                : 'text-slate-500 dark:text-brand-200/50'
                        }`}
                    >
                        {s}
                    </button>
            ...
            {loading ? (
                ...
                <div className="text-center py-16">
                    <Flag className="mx-auto text-slate-300 dark:text-brand-300/30 mb-3" size={28} />
                    <p className="text-sm text-slate-400 dark:text-brand-200/50">No {statusFilter} reports.</p>
                </div>
            ) : (
                <div className="space-y-2">
                    {reports.map((r) => (
                        <div key={r.id} className="bg-white dark:bg-ink-800 border border-slate-200 dark:border-ink-600 rounded-2xl p-4">
                            <div className="flex items-start justify-between gap-3 flex-wrap">
                                <div className="min-w-0">
                                    ...
                                    {r.product_title && (
                                        <p className="text-sm font-semibold text-slate-800 dark:text-brand-100 mt-1.5">
                                            Listing: {r.product_title}
                                        </p>
                                    )}
                                    {r.reported_user_name && (
                                        <p className="text-sm text-slate-600 dark:text-brand-100/80 mt-0.5">
                                            User: {r.reported_user_name}
                                        </p>
                                    )}
                                    <p className="text-xs text-slate-400 dark:text-brand-200/50 mt-1">
                                        Reported by {r.reporter_name} ({r.reporter_email})
                                    </p>
                                    {r.details && (
                                        <p className="text-sm text-slate-600 dark:text-brand-100/80 mt-2 bg-slate-50 dark:bg-ink-700 rounded-lg p-2.5">
                                            {r.details}
                                        </p>
                                    )}
                                </div>
...
function DeletedChatsTab() {
    ...
    if (chats.length === 0) {
        return (
            <div className="text-center py-16">
                <Trash2 className="mx-auto text-slate-300 dark:text-brand-300/30 mb-3" size={28} />
                <p className="text-sm text-slate-400 dark:text-brand-200/50">No chats have been deleted for everyone yet.</p>
            </div>
        );
    }
    ...
    return (
        <div className="max-w-3xl mx-auto">
            <p className="text-sm text-slate-400 dark:text-brand-200/50 mb-4">
            ...
            <div className="space-y-2">
                {chats.map((c) => (
                    ...
                    >
                        <div className="min-w-0">
                            <p className="text-sm font-semibold text-slate-800 dark:text-brand-100 truncate">
                                {c.buyer_name} ↔ {c.seller_name}
                            </p>
                            {c.product_title && (
                                <p className="text-xs text-slate-400 dark:text-brand-200/50 truncate">Re: {c.product_title}</p>
                            )}
                            <p className="text-xs text-slate-400 dark:text-brand-200/50 mt-1">
                                Deleted by <span className="font-semibold text-slate-600 dark:text-brand-200/70">{c.deleted_by_name}</span> ({c.deleted_by_email})
                            </p>
                        </div>
                        <span className="text-xs text-slate-400 dark:text-brand-200/50 shrink-0">
    ...
    return (
        <div className="fixed inset-0 z-[70] flex items-center justify-center px-4">
            ...
            <div className="relative bg-white dark:bg-ink-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-y-auto p-6 border border-slate-200 dark:border-ink-600">
                ...
                <h2 className="text-xl font-extrabold text-slate-900 dark:text-brand-50">Deleted conversation</h2>
                <p className="text-sm text-slate-500 dark:text-brand-200/50 mt-1">
                ...
                <div className="mt-4 bg-slate-50 dark:bg-ink-700 rounded-xl p-4 text-sm">
                    <p className="text-slate-500 dark:text-brand-200/60">
                        Deleted by <span className="font-semibold text-slate-800 dark:text-brand-100">{chat.deleted_by_name}</span> on{' '}
                        {new Date(chat.deleted_for_everyone_at).toLocaleString()}
                    </p>
                </div>

                <div className="mt-4">
                    <h3 className="font-bold text-slate-800 dark:text-brand-100 mb-2">
                    ...
                    {loading ? (
                        ...
                        <p className="text-sm text-slate-400 dark:text-brand-200/50">No messages found.</p>
                    ) : (
                                ...
                                return (
                                    <div key={m.id} className="bg-slate-50 dark:bg-ink-700 rounded-xl p-3">
                                        <div className="flex items-center justify-between gap-2">
                                            <span className="text-xs font-semibold text-slate-600 dark:text-brand-200/70">
                                                {isBuyer ? chat.buyer_name : chat.seller_name}
                                            </span>
                                            <span className="text-[11px] text-slate-400 dark:text-brand-200/50">
                                                {new Date(m.created_at).toLocaleString()}
                                            </span>
                                        </div>
                                        {m.content && (
                                            <p className="text-sm text-slate-700 dark:text-brand-100 mt-1">{m.content}</p>
                                        )}
...
function Tag({ children, color }) {
    const colors = {
        slate: 'bg-slate-100 dark:bg-ink-700 text-slate-600 dark:text-brand-200/60',
        gold: 'bg-amber-50 dark:bg-brand-900/40 text-amber-700 dark:text-brand-300',
        ...
        <button
            ...
            className={`p-2 rounded-lg transition ${className} ${
                active
                    ...
                    : 'text-slate-400 dark:text-brand-300/40 hover:bg-slate-100 dark:hover:bg-ink-700'
            }`}
        >
            {children}
        </button>
