    ...
    return (
        <>
            ...
            >
                ...
                >
                {/* Header */}
                <div className="relative bg-gradient-to-br from-brand-700 via-brand-600 to-accent-500 dark:from-ink-900 dark:via-ink-800 dark:to-brand-900 px-6 pt-6 pb-16">
                    <button
                        onClick={onClose}
                ...
                <div className="px-6 -mt-12">
                    <div className="relative w-24 h-24">
                        ...
                        >
                            {avatarPreview || user.avatar_url ? (
                                ...
                                <span className="text-[11px] font-semibold text-slate-400 dark:text-brand-300/50 text-center px-2 leading-tight">
                                    Upload photo
                                </span>
                            )}
                            <div
                    ...
                    <div className="mt-3">
                        <h2 className="text-lg font-extrabold text-slate-900 dark:text-brand-50">{user.name}</h2>
                        <p className="text-sm text-slate-500 dark:text-brand-200/60">{user.university_email}</p>
                        {user.school && (
                            <p className="text-xs text-slate-400 dark:text-brand-200/40 mt-0.5">{user.school}</p>
                        )}
                        ...
                        {!user.verified && (
                            <button
                                ...
                                className="mt-3 inline-flex items-center gap-1.5 text-[11px] sm:text-xs font-semibold text-yellow-600 dark:text-brand-400 hover:text-yellow-700 dark:hover:text-brand-300 underline underline-offset-2"
                            >
                                Verify your account →
                            </button>
                        )}
                ...
                <div className="px-6 mt-6 pb-8 space-y-2">
                    ...
                    <div className="pb-4 border-b border-slate-50 dark:border-ink-600">
                        <button
                            ...
                            className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-semibold text-slate-800 dark:text-brand-100 transition"
                        >
                        ...
                        {personalOpen && (
                            <div className="space-y-3">
                                {onCooldown && !editing && (
                                    <p className="flex items-center gap-1.5 text-xs text-amber-600 dark:text-brand-400">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="flex gap-2 pt-1">
                                            <button
                                                onClick={handleSave}
                                                disabled={saving}
                                                className="flex-1 py-2 rounded-xl bg-brand-600 dark:bg-brand-500 hover:bg-brand-700 dark:hover:bg-brand-400 text-white dark:text-ink-900 text-sm font-semibold transition disabled:opacity-60"
                                            >
                                                {saving ? 'Saving…' : 'Save changes'}
                                            </button>
                                            <button
                                                onClick={() => setEditing(false)}
                                                className="px-4 py-2 rounded-xl border border-slate-200 dark:border-ink-600 text-slate-600 dark:text-brand-200 text-sm font-semibold hover:bg-slate-50 dark:hover:bg-ink-700 transition"
                                            >
                                                Cancel
                                            </button>
                                        </div>
                                    ...
                                    {!editing && (
                                        <button
                                            ...
                                            className={`text-xs font-semibold ${
                                                onCooldown
                                                    ? 'text-slate-300 dark:text-brand-300/30 cursor-not-allowed'
                    ...
                    <button
                        ...
                        className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl bg-slate-100 dark:bg-ink-700 hover:bg-slate-50 dark:hover:bg-ink-600 text-sm font-semibold text-slate-800 dark:text-brand-100 transition"
                    >
                    ...
                    >
                        ...
                        {chatUnreadCount > 0 && (
                            <span className="ml-auto text-xs bg-accent-500 dark:bg-brand-500 text-white dark:text-ink-900 px-1.5 py-0.5 rounded-full">
                                {chatUnreadCount}
                            </span>
                        )}
                    </button>
                    ...
                    <div className="rounded-xl bg-slate-100 dark:bg-ink-700 overflow-hidden">
                        <button
                            onClick={() => setSupportOpen(!supportOpen)}
                            className="w-full flex items-center justify-between px-3 py-2.5 text-sm font-semibold text-slate-800 dark:text-brand-100 transition hover:bg-slate-50 dark:hover:bg-ink-600"
                        >
                            <div className="flex items-center gap-3">
                                <Info size={17} />
                        ...
                        {supportOpen && (
                            <div className="px-3 pb-3 space-y-1">
                                ...
                                <div className="rounded-lg bg-white/50 dark:bg-ink-800/50 overflow-hidden">
                                    ...
                                    >
                                        <Mail size={15} className="text-slate-400 dark:text-brand-300/50 shrink-0" />
                                        <span className="text-sm text-slate-700 dark:text-brand-100 flex-1">Contact support</span>
                                        <ChevronDown size={15} className={`text-slate-300 dark:text-brand-300/30 transition-transform shrink-0 ${contactOpen ? 'rotate-180' : ''}`} />
                                    </button>

                                    {contactOpen && (
                                        <div className="text-xs text-slate-500 dark:text-brand-200/60 space-y-1.5 px-3 pb-3 pl-9">
                                            <a
                                                href="tel:+233241234567"
                                                className="flex items-center gap-1.5 hover:text-brand-700 dark:hover:text-brand-300 transition w-fit"
                                            >
                                            ...
                                            >
                                                ...
                                                <span className="text-brand-600 dark:text-brand-400 font-semibold">@Trex_Support1</span>
                                            </a>
                                            <a 
                                ...
                                >
                                    <FileText size={15} className="text-slate-400 dark:text-brand-300/50 shrink-0" />
                                    <span className="text-sm text-slate-700 dark:text-brand-100">Terms of Service</span>
                                    <ChevronRight size={15} className="ml-auto text-slate-300 dark:text-brand-300/30" />
                                </Link>
                                ...
                                <Link
                                    ...
                                    className="w-full flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-white/50 dark:hover:bg-ink-800/50 transition text-left"
                                >
                                    <Shield size={15} className="text-slate-400 dark:text-brand-300/50 shrink-0" />
                                    <span className="text-sm text-slate-700 dark:text-brand-100">Privacy Policy</span>
                                    <ChevronRight size={15} className="ml-auto text-slate-300 dark:text-brand-300/30" />
                                </Link>
                            </div>
                        )}
                ...
                <div className="shrink-0 py-3 text-center border-t border-slate-100 dark:border-ink-700">
                    <span className="text-xs text-slate-400 dark:text-brand-300/40">App version v{APP_VERSION}</span>
                </div>
            </div>
            ...
            >
                ...
                >
                    <button
                        onClick={() => !uploading && setAvatarModalOpen(false)}
                        className="absolute top-3 right-3 p-1.5 rounded-lg text-slate-400 dark:text-brand-300/50 hover:bg-slate-100 dark:hover:bg-ink-700 transition"
                    >
                        <X size={18} />
                    </button>

                    <h3 className="text-sm font-bold text-slate-900 dark:text-brand-50 text-center mb-4">
                        Profile photo
                    </h3>
                    ...
                    <div className="w-64 h-64 mx-auto rounded-full overflow-hidden bg-slate-100 dark:bg-ink-700 flex items-center justify-center relative">                        {avatarPreview || user.avatar_url ? (
                            ...
                            <span className="text-xs font-semibold text-slate-400 dark:text-brand-300/50 text-center px-4">
                                No photo yet
                            </span>
                        )}
                        {uploading && (
                    ...
                    <button
                        ...
                        disabled={uploading}
                        className="w-full mt-5 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-brand-600 dark:bg-brand-500 hover:bg-brand-700 dark:hover:bg-brand-400 text-white dark:text-ink-900 text-sm font-semibold transition disabled:opacity-60"
                    >
                        {uploading ? (
                            <>
...
function Field({ icon, label, value, onChange, placeholder, as = 'input' }) {
    return (
        <div>
            <label className="text-xs font-semibold text-slate-500 dark:text-brand-300/60 flex items-center gap-1.5 mb-1">
                {icon} {label}
            </label>
            {as === 'textarea' ? (
                <textarea
                    ...
                    rows={3}
                    className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-ink-600 dark:bg-ink-700 dark:text-brand-50 dark:placeholder-brand-300/30 focus:border-brand-500 dark:focus:border-brand-500 focus:ring-2 focus:ring-brand-100 dark:focus:ring-brand-900 focus:outline-none text-sm transition resize-none"
                />
            ) : (
                <input
                    value={value}
                    onChange={(e) => onChange(e.target.value)}
                    placeholder={placeholder}
                    className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-ink-600 dark:bg-ink-700 dark:text-brand-50 dark:placeholder-brand-300/30 focus:border-brand-500 dark:focus:border-brand-500 focus:ring-2 focus:ring-brand-100 dark:focus:ring-brand-900 focus:outline-none text-sm transition"
                />
            )}
        </div>
    );
...
function InfoRow({ icon, label, value }) {
    return (
        <div className="flex items-start gap-3 px-3 py-2.5 rounded-xl bg-slate-50 dark:bg-ink-700">
            <span className="text-slate-400 dark:text-brand-300/50 mt-0.5">{icon}</span>
            <div>
                <p className="text-xs text-slate-400 dark:text-brand-200/40">{label}</p>
                <p className="text-sm font-medium text-slate-800 dark:text-brand-100">{value}</p>
            </div>
        </div>
    );
}