...
export default function VerifyModal({ open, onClose }) {
    ...
    // Lock body scroll while open. Self-aware — VerifyModal is opened from
    // inside ProfileDrawer (which already locks the body), so this only
    // takes/releases the lock if nothing already owns it.
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center px-4">
            ...
            <div className="relative bg-white dark:bg-ink-800 border border-transparent dark:border-ink-600 rounded-2xl shadow-2xl w-full max-w-sm p-6">
                <button
                    onClick={handleClose}
                    className="absolute top-4 right-4 p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-ink-700 text-slate-400 dark:text-brand-200/50 transition"
                >
                    <X size={18} />
                </button>

                <div className="w-11 h-11 rounded-xl bg-brand-50 dark:bg-brand-900 flex items-center justify-center mb-4">
                    <ShieldCheck className="text-brand-600 dark:text-brand-400" size={22} />
                </div>

                {step === 'start' ? (
                    <>
                        <h2 className="text-lg font-extrabold text-slate-900 dark:text-brand-50">Verify your account</h2>
                        ...
                        <div className="mt-4 flex items-center gap-2.5 bg-slate-50 dark:bg-ink-700 rounded-xl px-3.5 py-3">
                            <Mail size={16} className="text-slate-400 dark:text-brand-200/50 shrink-0" />
                            <span className="text-sm font-medium text-slate-700 dark:text-brand-100 truncate">
                        ...
                        <button
                            ...
                            disabled={sending}
                            className="w-full mt-5 py-2.5 rounded-xl bg-brand-600 dark:bg-brand-500 hover:bg-brand-700 dark:hover:bg-brand-400 text-white dark:text-ink-900 font-semibold text-sm transition disabled:opacity-60"
                        >
                    ...
                    <>
                        <h2 className="text-lg font-extrabold text-slate-900 dark:text-brand-50">Enter your code</h2>
                        <p className="text-sm text-slate-500 dark:text-brand-200/60 mt-1.5">
                        ...
                        <input
                            ...
                            className="w-full mt-4 px-4 py-3 rounded-xl border border-slate-200 dark:border-ink-600 dark:bg-ink-700 dark:text-brand-50 focus:border-brand-500 dark:focus:border-brand-500 focus:ring-2 focus:ring-brand-100 dark:focus:ring-brand-900 focus:outline-none text-center text-lg font-bold tracking-[0.3em] transition"
                        />
                        ...
                        <button
                            onClick={handleSendCode}
                            className="w-full mt-2 py-2 text-xs font-semibold text-slate-400 dark:text-brand-300/50 hover:text-slate-600 dark:hover:text-brand-100 transition"
                        >
