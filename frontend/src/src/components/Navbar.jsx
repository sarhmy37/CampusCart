...
const NOTIF_META = {
    ...
    new_listing: { emoji: '🛍️', color: 'bg-brand-50 text-brand-600 dark:bg-brand-900/40 dark:text-brand-400' },
    out_of_stock: { emoji: '📦', color: 'bg-slate-100 text-slate-500 dark:bg-ink-700 dark:text-brand-300' },
    ...
    default: { emoji: '🔔', color: 'bg-slate-100 text-slate-500 dark:bg-ink-700 dark:text-brand-300' },
};
...
const badgeClass = "absolute -top-1 -right-1 bg-accent-500 dark:bg-brand-500 text-white dark:text-ink-900 text-[10px] font-bold rounded-full min-w-[16px] h-4 px-1 flex items-center justify-center";
        ...
        <header className="sticky top-0 z-40 bg-white/90 dark:bg-ink-900/90 backdrop-blur border-b border-slate-200 dark:border-ink-600">
                ...
                <div className="flex items-center gap-1 sm:gap-1.5 shrink-0">
                    ...
                    {user && !isAdmin && (
                        <button
                            ...
                            title="Profile"
                        >
                            <Menu className="w-4 h-4 sm:w-5 sm:h-5 text-slate-700 dark:text-brand-200" />
                        </button>
                    )}
                    ...
                    >
                        ...
                        <div className="flex items-center font-serif font-black tracking-wider whitespace-nowrap">
                            <span className="text-base sm:text-lg text-slate-900 dark:text-brand-200">Tre</span>
                            <span className="text-base sm:text-lg text-slate-900 dark:text-brand-200 -ml-0.5">-</span>
                            <span className="text-2xl sm:text-3xl italic text-brand-600 dark:text-brand-400 leading-none -ml-1">X</span>
                        </div>
                ...
                <nav className="flex items-center gap-0.5 sm:gap-2 shrink-0">
                    ...
                    {user && !isAdmin && (
                        ...
                        >
                            {mobileExpanded ? (
                                <ChevronLeft className="w-4 h-4 text-slate-700 dark:text-brand-200" />
                            ) : (
                                <ChevronRight className="w-4 h-4 text-slate-700 dark:text-brand-200" />
                            )}
                        </button>
                    )}
                    ...
                    {user && !isAdmin && !isHome && (
                        <Link to="/" className="p-1.5 sm:p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-ink-700 transition" title="Home">
                            <Home className="w-4 h-4 sm:w-5 sm:h-5 text-slate-700 dark:text-brand-200" />
                        </Link>
                    )}
                    
                    {/* Sell button - ONLY for non-admin sellers */}
                    {user && user.account_type === 'seller' && !isAdmin && (
                        <Link to="/sell/new" className="hidden sm:flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-semibold text-red-700 dark:text-brand-400 hover:bg-brand-50 dark:hover:bg-ink-700 transition">
                    ...
                    {user && !isAdmin && (
                        <div ref={messagesRef} className={`relative ${mobileExpanded ? 'flex' : 'hidden'} sm:flex`}>
                            <button
                                ...
                                title="Messages"
                            >
                                <MessageCircle className="w-4 h-4 sm:w-5 sm:h-5 text-slate-700 dark:text-brand-200" />
                                {chatUnreadCount > 0 && (
                            ...
                            {showMessages && (
                                <div className="absolute right-0 mt-2 w-72 sm:w-80 max-h-96 overflow-y-auto bg-white dark:bg-ink-800 rounded-2xl shadow-xl border border-slate-200 dark:border-ink-600 z-50">
                                    <div className="px-4 py-3 border-b border-slate-100 dark:border-ink-600 font-semibold text-sm text-slate-900 dark:text-brand-100">
                                        Messages
                                    </div>
                                    {conversations.length === 0 ? (
                                        <p className="px-4 py-8 text-center text-sm text-slate-400 dark:text-brand-200/50">
                                                ...
                                                >
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        <div className="flex items-center justify-between gap-2">
                                                            <p className="text-sm font-semibold text-slate-900 dark:text-brand-100 truncate">
                                                                {c.other_user_name}
                                                            </p>
                                                            {c.unread_count > 0 && (
                                                                <span className="bg-accent-500 dark:bg-brand-500 text-white dark:text-ink-900 text-[10px] font-bold rounded-full min-w-[16px] h-4 px-1 flex items-center justify-center shrink-0">
                                                                    {formatBadgeCount(c.unread_count)}
                                                                </span>
                                                            )}
                                                        </div>
                                                        <p className="text-xs text-slate-500 dark:text-brand-200/60 truncate mt-0.5">
                                                            {c.last_message || 'No messages yet'}
                                                        </p>
                                                        {c.last_message_at && (
                                                            <p className="text-[11px] text-slate-400 dark:text-brand-300/40 mt-0.5">
                                            ...
                                            {conversations.length > visibleCount && (
                                                <div className="px-4 py-2 border-t border-slate-100 dark:border-ink-600">
                                                    <button
                                                        ...
                                                        className="text-xs text-brand-600 dark:text-brand-400 hover:text-brand-700 dark:hover:text-brand-300 transition font-semibold w-full text-center"
                                                    >
                                                        Show more
                                                    </button>
                                                </div>
                    ...
                    {user && (
                        <div ref={notificationsRef} className={`relative ${mobileExpanded ? 'flex' : 'hidden'} sm:flex`}>
                            <button
                                ...
                                title="Notifications"
                            >
                                <Bell className="w-4 h-4 sm:w-5 sm:h-5 text-slate-700 dark:text-brand-200" />
                                {unreadCount > 0 && (
                                        ...
                                        <>
                                            ...
                                            <div className="px-4 py-2 border-t border-slate-100 dark:border-ink-600">
                                                <button
                                                    ...
                                                    className="text-xs text-slate-400 dark:text-brand-200/50 hover:text-red-500 dark:hover:text-red-400 transition font-semibold w-full text-center"
                                                >
                                                    Clear all
                                                </button>
                                            </div>
                    ...
                    {user && !isAdmin && (
                        <Link to="/cart" className="relative p-1.5 sm:p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-ink-700 transition">
                            <ShoppingCart className="w-4 h-4 sm:w-5 sm:h-5 text-slate-700 dark:text-brand-200" />
                            {count > 0 && (
                    ...
                    {user ? (
                        <>
                            ...
                            {!isAdmin && (
                                <div ref={wishlistRef} className={`relative ${mobileExpanded ? 'block' : 'hidden'} sm:block`}>
                                    <button
                                        ...
                                        title="Wishlist"
                                    >
                                        <Heart className="w-4 h-4 sm:w-5 sm:h-5 text-slate-700 dark:text-brand-200" />
                                        {wishlistCount > 0 && (
                                                    ...
                                                    >
                                                        ...
                                                        >
                                                            ...
                                                            <div className="min-w-0">
                                                                <p className="text-sm font-semibold text-slate-900 dark:text-brand-100 truncate">{p.title}</p>
                                                                <p className="text-xs text-slate-400 dark:text-brand-200/50 truncate">
                                                                    GHS {parseFloat(p.price).toFixed(2)} · {p.condition}
                                                                </p>
                                                                <p className="text-xs text-slate-400 dark:text-brand-200/50 truncate">{p.seller_name}</p>
                                                            </div>
                                                        </Link>
                                                        <button
                            ...
                            {isAdmin && (
                                <Link 
                                    to="/admin" 
                                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-brand-50 dark:bg-brand-900/40 text-brand-700 dark:text-brand-400 text-sm font-semibold hover:bg-brand-100 dark:hover:bg-brand-900/60 transition"
                                >
                        ...
                        <>
                            {/* Desktop — unchanged */}
                            <Link to="/login" className="hidden sm:inline-block px-3 py-2 text-sm font-semibold text-slate-700 dark:text-brand-200 hover:text-brand-700 dark:hover:text-brand-100 transition whitespace-nowrap">Log in</Link>
                            <Link to="/register" className="hidden sm:inline-block px-4 py-2 text-sm font-semibold text-white dark:text-ink-900 bg-brand-600 dark:bg-brand-500 hover:bg-brand-700 dark:hover:bg-brand-400 rounded-lg transition shadow-sm whitespace-nowrap">Sign up</Link>
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
                ...
                <div className="relative bg-white dark:bg-ink-800 rounded-2xl shadow-2xl max-w-sm w-full p-6 border border-slate-200 dark:border-ink-600">
                    <div className="text-center">
                        ...
                        <h3 className="font-extrabold text-slate-900 dark:text-brand-50 text-lg">Logout of Admin?</h3>
    ...
    return (
        <form onSubmit={onSubmit} className="w-full">
            <input
                ...
                className="w-full px-3 py-2 sm:px-4 sm:py-2.5 rounded-full bg-slate-100 dark:bg-ink-700 border border-transparent dark:text-brand-50 dark:placeholder-brand-300/40 focus:border-brand-400 dark:focus:border-brand-500 focus:bg-white dark:focus:bg-ink-700 focus:outline-none text-xs sm:text-sm transition"
            />
        </form>
    );
}
    ...
    return (
        ...
        >
                ...
                {visible.map((i) => (
                    <div
                        ...
                        className={`shrink-0 h-full flex items-center justify-center text-xs font-semibold ${
                            label(i) === 'Log in'
                                ? 'text-slate-700 dark:text-brand-200'
                                : 'text-white dark:text-ink-900'
                        }`}
                    >
                        <span
                            className={label(i) === 'Sign up' ? 'px-2.5 py-1 rounded-lg bg-brand-600 dark:bg-brand-500' : ''}
                        >
                            {label(i)}
                        </span>
                    </div>
            ...
            <div className="pointer-events-none absolute inset-y-0 left-0 w-4 bg-gradient-to-r from-slate-50 dark:from-ink-800 to-transparent flex items-center">
                <ChevronLeft size={10} className="text-slate-300 dark:text-brand-300/30" />
            </div>
            <div className="pointer-events-none absolute inset-y-0 right-0 w-4 bg-gradient-to-l from-slate-50 dark:from-ink-800 to-transparent flex items-center justify-end">
                <ChevronRight size={10} className="text-slate-300 dark:text-brand-300/30" />
            </div>
        </div>
    );
}
    ...
    return (
        <div className="relative overflow-hidden border-b border-slate-50 dark:border-ink-700 last:border-0">
            ...
            >
                ...
                <div className="min-w-0 flex-1">
                    <p className="text-sm font-semibold text-slate-900 dark:text-brand-100 leading-snug">
                        {n.title}
                    </p>
                    {n.message && (
                        <p className="text-xs text-slate-500 dark:text-brand-200/60 mt-0.5 leading-relaxed">
                            {n.message}
                        </p>
                    )}
                    {n.created_at && (
                        <p className="text-[11px] text-slate-400 dark:text-brand-300/40 mt-1">
