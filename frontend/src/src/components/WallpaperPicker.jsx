...
export default function WallpaperPicker({ open, onClose, currentUserId }) {
    ...
    // Lock body scroll while open. Self-aware — WallpaperPicker is opened
    // from inside ChatPanel (which already locks the body), so this only
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-slate-900/50 backdrop-blur-sm">
            <div className="bg-white dark:bg-ink-800 rounded-2xl shadow-2xl w-full max-w-md p-6 relative max-h-[85vh] overflow-y-auto">
                ...
                <h3 className="text-lg font-extrabold text-slate-900 dark:text-brand-50">Chat wallpaper</h3>
                ...
                <div className="grid grid-cols-4 gap-3 mt-5">
                    {WALLPAPER_PRESETS.map((preset) => (
                        ...
                        >
                            ...
                            >
                                {isHighlighted(preset) && (
                                    <div className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-brand-600 dark:bg-brand-500 flex items-center justify-center">
                                        <Check size={12} className="text-white dark:text-ink-900" />
                                    </div>
                                )}
                            </div>
                            <span className="text-[11px] text-slate-500 dark:text-brand-200/60">{preset.label}</span>
                        </button>
                    ))}
                </div>
                ...
                <div className="mt-5 pt-5 border-t border-slate-100 dark:border-ink-600">
                    ...
                    <button
                        ...
                        className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border text-sm font-semibold transition disabled:opacity-60 ${
                            staged?.kind === 'custom'
                                ...
                                : 'border-dashed border-slate-300 dark:border-ink-600 text-slate-600 dark:text-brand-200/70 hover:bg-slate-50 dark:hover:bg-ink-700'
                        }`}
                    >
                        <Upload size={16} />
                    ...
                    {wallpaper?.type === 'custom' && !staged && (
                        <p className="text-xs text-slate-400 dark:text-brand-200/50 mt-2 text-center">
                ...
                <button
                    ...
                    className="w-full mt-5 py-2.5 rounded-xl bg-brand-600 dark:bg-brand-500 hover:bg-brand-700 dark:hover:bg-brand-400 text-white dark:text-ink-900 font-semibold text-sm transition disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                    {busy ? (
                        <>
                ...
                {someoneElseSetIt && !staged && (
                    <button
                        ...
                        disabled={togglingHide}
                        className="w-full mt-2 py-2.5 rounded-xl border border-slate-200 dark:border-ink-600 text-slate-600 dark:text-brand-200/70 font-semibold text-sm hover:bg-slate-50 dark:hover:bg-ink-700 transition disabled:opacity-60"
                    >
                        {togglingHide
                            ? 'Updating…'
