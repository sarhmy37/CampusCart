...
export default function ProductCard({ product }) {
    ...
    let stockLabel = null;
    let stockColor = 'text-slate-400 dark:text-brand-200/50';
    if (stock !== null) {
        if (stock <= 0) {
    ...
    const CardInner = (
        <div 
            ref={cardRef}
            className="group relative bg-white dark:bg-ink-800 rounded-xl border border-slate-200 dark:border-ink-600 overflow-hidden hover:shadow-lg dark:hover:shadow-brand-900/20 hover:-translate-y-0.5 transition-all duration-300"
        >
            <div className="aspect-square bg-slate-100 dark:bg-ink-700 overflow-hidden relative">
                ...
                <span className="absolute top-1.5 left-1.5 bg-white/90 dark:bg-ink-900/80 backdrop-blur px-1.5 py-0.5 rounded-full text-[9px] font-semibold text-slate-700 dark:text-brand-100 capitalize shadow-sm">
                    {product.condition}
                </span>
                ...
                <button
                    ...
                    className={`absolute bottom-0 right-0 w-6 h-6 flex items-center justify-center transition ${
                        wishlisted ? 'text-red-500' : 'text-slate-400 dark:text-brand-200/60 hover:text-red-500'
                    }`}
                >
            ...
            <div className="p-2.5">
                ...
                <h3 className="font-semibold text-slate-900 dark:text-brand-50 text-xs line-clamp-2 leading-snug">
                    {product.title}
                </h3>
                ...
                <div className="flex items-center gap-1 mt-0.5">
                    ...
                    <span className="text-[11px] font-semibold text-slate-700 dark:text-brand-100">{rating ? Number(rating).toFixed(1) : 'New'}</span>
                    {reviewCount > 0 && (
                        <span className="text-[11px] text-slate-400 dark:text-brand-200/50">({reviewCount})</span>
                    )}
                </div>
                    ...
                    {discountPercent !== null ? (
                        <>
                            <div className="flex items-center gap-1.5">
                                <p className="text-[10px] text-slate-400 dark:text-brand-200/50 line-through">
                            ...
                            <span className="text-brand-700 dark:text-brand-400 font-extrabold text-xs">
                                GHS {currentPrice.toFixed(2)}
                            </span>
                        </>
                    ) : (
                        <span className="text-brand-700 dark:text-brand-400 font-extrabold text-sm">
                            GHS {currentPrice.toFixed(2)}
                        </span>
                    )}
                </div>

                <p className="text-[11px] text-slate-400 dark:text-brand-200/50 mt-0.5 truncate flex items-center gap-1">
                    {product.seller_name}
