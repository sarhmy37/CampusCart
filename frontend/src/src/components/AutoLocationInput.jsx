    ...
    return (
        <div className="relative w-full">
            <div 
                ...
                className={`w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-ink-600 dark:bg-ink-700 flex items-center justify-between cursor-pointer transition hover:border-brand-400 dark:hover:border-brand-500 ${loading ? 'opacity-80' : ''}`}
            >
                <span className={`text-sm ${value ? 'text-slate-800 dark:text-brand-50' : 'text-slate-400 dark:text-brand-300/40'}`}>
                    {value || placeholder}
                </span>
                {loading ? (
                    <Loader2 className="w-5 h-5 text-brand-600 dark:text-brand-400 animate-spin" />
                ) : (
                    <MapPin className="w-5 h-5 text-slate-400 dark:text-brand-300/40" />
                )}
            </div>
        </div>
    );
