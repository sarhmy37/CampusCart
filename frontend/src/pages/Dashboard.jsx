...
<div className="absolute inset-0 bg-gradient-to-br from-ink-900/80 via-ink-800/55 to-brand-600/35 dark:from-ink-900/90 dark:via-ink-900/75 dark:to-gold-900/50" />                <div className="absolute -right-16 -top-20 w-72 h-72 bg-white/10 rounded-full blur-2xl" />
...
function PayoutSettings({ period }) {
    ...
    const [showBalancePasswordModal, setShowBalancePasswordModal] = useState(false);
    const [balancePassword, setBalancePassword] = useState('');
    const [verifyingBalancePassword, setVerifyingBalancePassword] = useState(false);
    ...
    const handleToggleBalance = () => {
        ...
        setBalancePassword('');
        setShowBalancePasswordModal(true);
    };

    const handleVerifyBalancePassword = async () => {
        if (!balancePassword) {
            toast.error('Enter your password');
            return;
        }
        setVerifyingBalancePassword(true);
        try {
            await api.post('/auth/me/verify-password', { password: balancePassword });
            setBalanceRevealed(true);
            setShowBalancePasswordModal(false);
            setBalancePassword('');
        } catch (err) {
            toast.error(err.response?.data?.error || 'Incorrect password');
        } finally {
            setVerifyingBalancePassword(false);
        }
    };
            ...
            {showBalancePasswordModal && createPortal(
                <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-slate-900/50 backdrop-blur-sm">
                    <div className="bg-white dark:bg-ink-800 rounded-2xl shadow-2xl w-full max-w-sm p-6 relative">
                        ...
                        <h3 className="text-lg font-extrabold text-slate-900 dark:text-gold-50">Confirm it's you</h3>
                        <p className="text-sm text-slate-500 dark:text-gold-200/50 mt-1">
                            Enter your password to view your available balance.
                        </p>
                        ...
                        <button
                            ...
                            disabled={verifyingBalancePassword}
                        ...
                        >
                            {verifyingBalancePassword ? 'Verifying…' : 'Show balance'}
                        </button>
                    </div>
                </div>,
                document.body
