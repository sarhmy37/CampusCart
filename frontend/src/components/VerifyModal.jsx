    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center px-4">
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
                ...
                {step === 'start' ? (
                    <>
                        <h2 className="text-lg font-extrabold text-slate-900">Verify your account</h2>
                        <p className="text-sm text-slate-500 mt-1.5">
                            We'll email a 6-digit code to confirm this is really you.
                        </p>

                        <div className="mt-4 flex items-center gap-2.5 bg-slate-50 rounded-xl px-3.5 py-3">
                        ...
                        <button
                            ...
                            disabled={sending}
                            className="w-full mt-5 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-700 text-white font-semibold text-sm transition disabled:opacity-60"
                        >
                    ...
                    <>
                        ...
                        <input
                            ...
                            className="w-full mt-4 px-4 py-3 rounded-xl border border-slate-200 focus:border-brand-500 focus:ring-2 focus:ring-brand-100 focus:outline-none text-center text-lg font-bold tracking-[0.3em] transition"
                        />

                        <button
                            onClick={handleVerify}
                            disabled={verifying || inputCode.length !== 6}
                            className="w-full mt-4 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-700 text-white font-semibold text-sm transition disabled:opacity-60"
                        >
                            {verifying ? 'Verifying…' : 'Verify'}
                        </button>
                        <button
                            onClick={handleSendCode}
                            className="w-full mt-2 py-2 text-xs font-semibold text-slate-400 hover:text-slate-600 transition"
                        >
