        ...
        <div className="mt-6 bg-white dark:bg-ink-800 border border-slate-200 dark:border-ink-600 rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                    <h2 className="font-bold text-slate-900 dark:text-gold-50">Reviews</h2>
                    {data?.avg_rating && (
                        <span className="flex items-center gap-1 text-sm">
                            <Star size={14} className="fill-amber-400 text-amber-400" />
                            <span className="font-semibold text-slate-800 dark:text-gold-100">{data.avg_rating}</span>
                            <span className="text-slate-400 dark:text-gold-200/50">({data.total})</span>
                        </span>
                    )}
                </div>
                {user && (
            ...
            {!data?.reviews?.length ? (
                <p className="text-sm text-slate-400 dark:text-gold-200/50">No reviews yet for this product.</p>
            ) : (
    ...
    return (
        <div className="border-t border-slate-100 dark:border-ink-600 pt-4 first:border-0 first:pt-0">
            <div className="flex items-center justify-between">
                <p className="font-semibold text-slate-800 dark:text-gold-100 text-sm">{review.reviewer_name || 'Anonymous'}</p>
                ...
                <button
                    ...
                    className={`inline-flex items-center gap-1.5 text-xs font-semibold transition ${
                        review.liked_by_me
                            ...
                            : 'text-slate-400 dark:text-gold-200/40 hover:text-slate-600 dark:hover:text-gold-200'
                    }`}
                >
            ...
            {showComments && (
                <div className="mt-3 pl-3 border-l-2 border-slate-100 dark:border-ink-600 space-y-2.5">
                    {review.comments?.map((c) => (
                        <div key={c.id}>
                            <p className="text-xs font-semibold text-slate-700 dark:text-gold-200">{c.commenter_name}</p>
                            <p className="text-xs text-slate-500 dark:text-gold-200/60">{c.content}</p>
                        </div>
                    ))}
                    {user && (
                        <form onSubmit={handleComment} className="flex items-center gap-2 pt-1">
                            <input
                                ...
                                className="flex-1 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-ink-600 dark:bg-ink-700 dark:text-gold-50 focus:border-brand-500 dark:focus:border-gold-500 focus:outline-none text-xs transition"
                            />
                            <button
                                type="submit"
                                disabled={submitting || !commentText.trim()}
                                className="text-brand-600 dark:text-gold-400 disabled:opacity-40 transition"
                            >
                                <Send size={15} />
                            </button>
                        </form>
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center px-4">
            ...
            <div className="relative bg-white dark:bg-ink-800 rounded-2xl shadow-2xl w-full max-w-sm p-6">
                ...
                <h2 className="text-lg font-extrabold text-slate-900 dark:text-gold-50">Rate this product</h2>
                <p className="text-sm text-slate-500 dark:text-gold-200/50 mt-1">Based on your completed purchase.</p>
